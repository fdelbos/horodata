...
func NewAddress(c *gin.Context) {
	...
	if data.Name == "" {
		...
		errors["name"] = "Ce champ ne doit pas dépasser plus de 50 caractères."
	}
	...
	if len(data.Company) > 100 {
		errors["company"] = "Ce champ ne doit pas dépasser plus de 100 caractères."
	}

	data.VAT = strings.TrimSpace(data.VAT)
	if len(data.VAT) > 25 {
		errors["vat"] = "Ce champ ne doit pas dépasser plus de 25 caractères."
	}
	...
	if data.Address1 == "" {
		...
		errors["addr1"] = "Ce champ ne doit pas dépasser plus de 150 caractères."
	}

	data.Address2 = strings.TrimSpace(data.Address2)
	if len(data.Address2) > 150 {
		errors["addr2"] = "Ce champ ne doit pas dépasser plus de 150 caractères."
	}
	...
	if data.City == "" {
		...
		errors["city"] = "Ce champ ne doit pas dépasser plus de 100 caractères."
	}
