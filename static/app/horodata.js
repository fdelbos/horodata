...
$templateCache.put("horodata/widgets/empty_group.html","<div layout-padding layout-margin layout=column layout-align=\"center center\"><div ng-if=\"group.tasks.length == 0 && group.customers.length == 0\"><div class=\"md-headline input-error\">Le groupe n\'est pas encore configuré\'</div><p>Rendez vous sous l\'onglet \"Configuration\" pour ajouter des dossiers et tâches à votre groupe, et, pour inviter d\'autres utilisateurs à saisir leur activité.</p></div><div ng-if=\"group.tasks.length > 0 && group.customers.length == 0\"><div class=\"md-headline input-error\">Le groupe ne possède aucun dossier</div><p>Vous devez ajouter des dossiers pour rendre possible la saisie. Cliquez sous l\'onglet \"Configuration\" pour en ajouter.</p></div><div ng-if=\"group.tasks.length == 0 && group.customers.length > 0\"><div class=\"md-headline input-error\">Le groupe ne possède aucune tâche</div><p>Vous devez ajouter des tâches pour rendre possible la saisie. Cliquez sous l\'onglet \"Configuration\" pour en ajouter.</p></div></div>");
...
$templateCache.put("horodata/widgets/new_group_form.html","<md-dialog aria-label=\"Créer un groupe\" flex=40><form name=newGroupForm><app-widgets-common-dialog-toolbar>Créer un groupe</app-widgets-common-dialog-toolbar><md-dialog-content><div class=md-dialog-content><app-widgets-common-quota-error></app-widgets-common-quota-error><p class=md-body-1>Un groupe c \'est du blabla...</p><md-input-container class=md-block ng-class=\"{\'md-input-invalid\': errors.name}\"><label>Nom du groupe</label> <input type=text ng-model=name> <small ng-if=errors.name class=input-error>{{ errors.name }}</small></md-input-container></div></md-dialog-content><app-widgets-common-dialog-actions><md-button ng-click=send() class=\"md-primary md-raised\">Créer</md-button></app-widgets-common-dialog-actions></form></md-dialog>");
...
$templateCache.put("horodata/views/billing/card.html","<div layout-padding><div layout=column flex-md=70 flex-gt-md=60><div layout=row layout-align=\"space-between center\"><div layout=row layout-align=\"center center\" hide show-gt-sm><md-icon class=\"md-36 md-accent\">credit_card</md-icon>&nbsp; <span class=md-display-1>Carte de crédit</span></div><div layout=row layout-align=\"center center\" hide-gt-sm><md-icon class=md-accent>credit_card</md-icon>&nbsp; <span class=md-headline>Carte de crédit</span></div><md-button ng-click=editCard($event) class=\"md-raised md-primary\" hide show-gt-sm><md-tooltip md-direction=top>Editer la carte de crédit</md-tooltip><md-icon class=md-18>edit</md-icon>Editer</md-button><md-button ng-click=editCard($event) class=\"md-fab md-mini md-primary\" hide-gt-sm aria-label=\"Editer la carte de crédit\"><md-icon class=md-24>edit</md-icon></md-button></div><p class=md-body-1>Cette carte sera utilisée pour les prélèvements mensuels.<br></p><app-widgets-common-loading ng-if=loading></app-widgets-common-loading><md-whiteframe ng-if=!loading class=md-whiteframe-1dp flex layout-padding><div ng-if=!card.current layout=row layout-align=\"center center\" flex><span>Vous n\'avez pas saisi de carte de crédit.</span></div><div ng-if=card.current layout=column><div layout=column flex><span class=\"md-caption text-grey\">Marque</span><div>{{ card.current.brand }}</div></div><br><div layout=column flex><span class=\"md-caption text-grey\">Numéro</span><div>#### #### #### {{ card.current.last4 }}</div></div><br><div layout=column flex><span class=\"md-caption text-grey\">Expiration</span><div>{{ card.current.expiration | Month }}</div></div></div></md-whiteframe></div></div>");
$templateCache.put("horodata/views/billing/card_edit.html","<md-dialog aria-label=\"Carte de crédit\" flex=50><form name=billingCard><app-widgets-common-dialog-toolbar>Carte de crédit</app-widgets-common-dialog-toolbar><md-dialog-content><div class=md-dialog-content><div layout-xs=column layout=row><md-input-container flex-xs flex-gt-xs=80 class=md-block ng-class=\"{\'md-input-invalid\': errors.number}\"><label>Numéro de carte</label> <input type=text md-maxlength=20 ng-model=card.new.number> <small ng-if=errors.number class=input-error>{{ errors.number }}</small></md-input-container><md-input-container flex-xs flex-gt-xs=20 class=md-block ng-class=\"{\'md-input-invalid\': errors.cvc}\"><label>CVC</label> <input type=text md-maxlength=4 ng-model=card.new.cvc> <small ng-if=errors.cvc class=input-error>{{ errors.cvc }}</small></md-input-container></div><div layout-xs=column layout=row><md-input-container flex=50 flex-xs=100 class=md-block ng-class=\"{\'md-input-invalid\': errors.exp_month}\"><label>Mois d\'expiration</label><md-select ng-model=card.new.exp_month><md-option ng-repeat=\"m in months\" value=\"{{ m.id }}\">{{ m.label }}</md-option></md-select><small ng-if=errors.exp_month class=input-error>{{ errors.exp_month }}</small></md-input-container><md-input-container flex=50 flex-xs=100 class=md-block ng-class=\"{\'md-input-invalid\': errors.exp_year}\"><label>Année d\'expiration</label><md-select ng-model=card.new.exp_year><md-option ng-repeat=\"y in years\" value=\"{{ y }}\">{{ y }}</md-option></md-select><small ng-if=errors.exp_year class=input-error>{{ errors.exp_year }}</small></md-input-container></div><div layout-align=\"center center\" layout=column><p class=\"text-grey md-body-1\">Notez que le numero de votre carte de credit ne transite jamais par nos serveurs. Le processus de paiement est entièrement géré par <a target=_blank href=https://stripe.com>stripe.com<md-icon class=md-14>open_in_new</md-icon></a></p></div></div></md-dialog-content><app-widgets-common-dialog-actions><md-button ng-click=update() class=\"md-primary md-raised\">Enregistrer</md-button></app-widgets-common-dialog-actions></form></md-dialog>");
$templateCache.put("horodata/views/billing/plan.html","<md-card><md-card-content><span class=md-headline>{{ plan.name }} <span ng-if=\"current == plan.code\" class=\"md-body-1 text-accent\">(Plan courant)</span></span><br><br><div><md-divider></md-divider><div layout-align=\"space-between center\" layout=row><span class=md-body-1>Utilisateurs</span> <span class=md-body-2>{{ plan.guests }}</span></div><md-divider></md-divider><div layout-align=\"space-between center\" layout=row><span class=md-body-1>Groupes</span> <span class=md-body-2>{{ plan.groups }}</span></div><md-divider></md-divider><div layout-align=\"space-between center\" layout=row><span class=md-body-1>Saisies (par jour)</span> <span class=md-body-2>{{ plan.jobs }}</span></div><br><div ng-if=\"plan.price != 0\" layout=column layout-align=\"center center\"><div layout-align=\"center center\" layout=row><span class=md-display-1>{{ plan.price }} &euro;</span> <span class=text-grey>&nbsp; / mois</span></div><span class=\"md-caption text-grey\">hors taxes</span></div><div ng-if=\"plan.price == 0\" layout=column layout-align=\"center center\"><div layout-align=\"center center\" layout=row><span class=md-display-1>Gratuit</span></div><span class=\"md-caption text-grey\">Pour toujours</span></div></div></md-card-content><md-card-actions layout=row layout-align=\"end center\"><md-button ng-if=\"current != plan.code\" ng-click=select($event) hide show-gt-sm class=\"md-raised md-primary\"><md-icon>check</md-icon>Selectionner</md-button><md-button ng-if=\"current != plan.code\" ng-click=select($event) hide-gt-sm class=\"md-fab md-primary\"><md-icon>check</md-icon></md-button><md-button ng-if=\"current == plan.code\" hide show-gt-sm class=md-raised ng-disabled=true><md-icon>check</md-icon>Plan courant</md-button><md-button ng-if=\"current == plan.code\" hide-gt-sm class=\"md-fab md-primary\" ng-disabled=true><md-icon>check</md-icon></md-button></md-card-actions></md-card>");
$templateCache.put("horodata/views/billing/plan_change.html","<md-dialog aria-label=\"Changer de plan\" flex=50><form name=planChange><app-widgets-common-dialog-toolbar>Changer de plan</app-widgets-common-dialog-toolbar><md-dialog-content><div class=md-dialog-content><app-widgets-common-loading ng-if=preLoading></app-widgets-common-loading><billing-plan-not-ready ng-if=\"!preLoading && !ready\"></billing-plan-not-ready><div ng-if=\"!preLoading && ready && !done\"><div ng-if=\"plan.code !=\'free\'\" layout=column><h3 class=md-headline>Plan selectionne: <span class=text-accent>{{ plan.name }}</span></h3><md-input-container class=md-block ng-class=\"{\'md-input-invalid\': errors.accept}\"><md-switch ng-model=form.accept aria-label=\"J\'ai lu et j\'accepte les conditions generales de ventes\">J\'ai lu et j\'accepte les <a href=/static/docs/cgv.pdf target=_blank>conditions generales de ventes<md-icon class=md-18>open_in_new</md-icon></a></md-switch><small ng-if=errors.accept class=input-error>{{ errors.accept }}</small></md-input-container><p>Votre nouveau plan sera effectif immediatement.</p></div><div ng-if=\"plan.code ==\'free\'\" layout=column><h3 class=md-headline>Souhaitez-vous vraiment revenir au plan gratuit?</h3><p>Votre carte de credit ne sera plus debitee mais vous ne serez pas rembourse pour le mois courant.</p><p>Votre quota sera mis a jour a la fin de la periode de facturation (le {{ period.end | Day }}).</p></div></div><div ng-if=done layout=column flex layout-align=\"center center\" layout-padding><h3 class=md-headline>Felicitations, votre abonnement a bien ete mis a jour.</h3><p>Merci pour votre confiance.</p></div></div></md-dialog-content><app-widgets-common-dialog-actions ng-if=!preLoading><md-button ng-if=\"ready && !done\" ng-click=validate() class=\"md-primary md-raised\">Valider</md-button></app-widgets-common-dialog-actions></form></md-dialog>");
$templateCache.put("horodata/views/billing/plan_not_ready.html","<div flex layout-padding><div ng-if=\"!addr.current && !card.current\" layout=column flex layout-align=\"center center\"><h3 class=md-headline>Vous n\'avez pas saisi d\'adresse ni de carte de crédit</h3><p>Avant de pouvoir changer votre plan, vous devez saisir une adresse et une carte de crédit.</p></div><div ng-if=\"addr.current && !card.current\" layout=column flex layout-align=\"center center\"><h3 class=md-headline>Vous n\'avez pas saisi de carte de crédit</h3><p>Avant de pouvoir changer votre plan, vous devez saisir une carte de crédit.</p></div><div ng-if=\"!addr.current && card.current\" layout=column flex layout-align=\"center center\"><h3 class=md-headline>Vous n\'avez pas saisi d\'adresse</h3><p>Avant de pouvoir changer votre plan, vous devez saisir une adresse.</p></div></div>");
$templateCache.put("horodata/views/billing/root.html","<div><billing-addr></billing-addr><md-divider></md-divider><billing-card></billing-card><md-divider></md-divider><app-widgets-common-loading ng-if=loading></app-widgets-common-loading><div ng-if=!loading flex-md=70 flex-gt-md=60 layout-padding><div layout=row><div layout=row layout-align=\"center center\" hide show-gt-sm><md-icon class=\"md-36 md-accent\">euro_symbol</md-icon>&nbsp; <span class=md-display-1>Plans</span></div><div layout=row layout-align=\"center center\" hide-gt-sm><md-icon class=md-accent>euro_symbol</md-icon>&nbsp; <span class=md-headline>Plans<span></span></span></div></div><p class=md-body-1>Tous nos plans sont sans engagement.</p><md-content class=md-padding layout-xs=column layout=row><div flex-xs flex-gt-xs=50 layout=column><billing-plan plan=plans.free current=plans.current></billing-plan><billing-plan plan=plans.medium current=plans.current></billing-plan></div><div flex-xs flex-gt-xs=50 layout=column><billing-plan plan=plans.small current=plans.current></billing-plan><billing-plan plan=plans.large current=plans.current></billing-plan></div></md-content></div><md-divider></md-divider><billing-invoices></billing-invoices></div>");}]);
...
angular.module("horodata").directive("billingPlan", [
  "popupService", function(popupService) {
    var l;
    l = function(scope, elem, attr) {
      return scope.select = function(ev) {
        return popupService("horodata/views/billing/plan_change.html", "BillingPlanChange", scope, ev);
      };
    };
    return {
      link: l,
...
angular.module("horodata").controller("BillingPlanChange", [
  "$scope", "$mdDialog", "$mdToast", "$http", "$location", "apiService", "groupService", function($scope, $mdDialog, $mdToast, $http, $location, apiService, groupService) {
    var getAddr, getCard, getEndPeriod, w;
    $scope.loading = false;
    ...
    getCard = function() {
      return $http.get((apiService.get()) + "/billing/card").then(function(resp) {
        ...
        return $scope.card.loaded = true;
      });
    };
    getCard();
    if ($scope.plan.code === "free") {
      getEndPeriod = function() {
        return $http.get((apiService.get()) + "/billing/end_period").then(function(resp) {
          $scope.period.loaded = true;
          return $scope.period.end = resp.data.data.end;
        }, function(resp) {
          return $scope.period.loaded = true;
        });
      };
      getEndPeriod();
      w = $scope.$watchGroup(["addr.loaded", "card.loaded", "period.loaded"], function(v) {
        if (!v[0] || !v[1] || !v[2]) {
          return;
        }
        $scope.preLoading = false;
        $scope.ready = ($scope.addr.current != null) && ($scope.card.current != null);
        return w();
      });
    } else {
      w = $scope.$watchGroup(["addr.loaded", "card.loaded"], function(v) {
        if (!v[0] || !v[1]) {
          return;
        }
        $scope.preLoading = false;
    ...
    return $scope.validate = function() {
      if ($scope.plan.code !== "free" && $scope.form.accept === false) {
        $scope.errors = {
      ...
      $scope.loading = true;
      return $http.post((apiService.get()) + "/billing/change_plan", {
        plan: $scope.plan.code
      }).then(function(resp) {
        $scope.loading = false;
        $scope.done = true;
        if ($scope.plan.code !== "free") {
          return $scope.current = $scope.plan.code;
        }
      }, function(resp) {
        return $scope.loading = false;
      });
...
angular.module("horodata").controller("Billing", [
  "$scope", "$http", "apiService", "titleService", function($scope, $http, apiService, titleService) {
    ...
    $scope.loading = true;
    return $http.get((apiService.get()) + "/billing/plan").then(function(resp) {
      $scope.loading = false;
      return $scope.plans.current = resp.data.data.plan;
    });
  }
]);
