...
*/
angular.module("horodata", ["ngMaterial", "ngRoute", "ngMessages", "infinite-scroll"]).config([
...
$templateCache.put("horodata/widgets/listing.html","<div><div ng-if=!listing.list layout=column layout-align=\"center center\"><md-progress-circular md-mode=indeterminate md-diameter=150></md-progress-circular><p>Chargement, veuillez patienter.</p></div><md-list class=md-body-1><md-list-item class=md-body-2><div layout=row layout-align=\"space-between center\" flex><div flex=20>Jour</div><div flex=20>Utilisateur</div><div flex=20>Dossier</div><div flex=20>Tâche</div><div flex=10>Duree</div></div><md-divider></md-divider></md-list-item><md-list-item ng-repeat=\"i in listing.list\" class=secondary-button-padding ng-click=alert($event)><div layout=row layout-align=\"space-between center\" flex><div flex=20>{{ i.created | Day }}</div><div flex=20>{{ i.creator.full_name }}</div><div flex=20>{{ customers[i.customer_id] }}</div><div flex=20>{{ tasks[i.task_id] }}</div><div flex=10>{{ i.duration | Duration }}</div></div><md-divider></md-divider></md-list-item></md-list></div>");
...
$templateCache.put("horodata/widgets/search_bar.html","<div><div layout=row layout-align=\"space-around center\" hide show-gt-md layout-padding><md-datepicker flex=25 ng-model=search.begin md-placeholder=\"Date debut\" md-max-date=search.end></md-datepicker><md-datepicker flex=25 ng-model=search.end md-placeholder=\"Date fin\" md-max-date=today></md-datepicker><md-input-container flex=25><label>Dossier</label><md-select ng-model=search.customer><md-option ng-repeat=\"c in group.customers\" value=\"{{ c.id }}\">{{ c.name }}</md-option></md-select></md-input-container><md-input-container flex=25><label>Utilisateur</label><md-select ng-model=search.guest><md-option ng-repeat=\"u in users\" value=\"{{ u.email }}\">{{ u.name }}</md-option></md-select></md-input-container></div><md-divider></md-divider></div>");
  ...
  "$scope", "$mdDialog", "$mdToast", "$http", "$location", "apiService", function($scope, $mdDialog, $mdToast, $http, $location, apiService) {
    ...
    return $scope.send = function() {
      ...
      return $http.post((apiService.get()) + "/groups/" + $scope.group.url + "/jobs", task).then(function(resp) {
        $mdDialog.hide();
        return $mdToast.showSimple("Nouvelle tâche ajoutee.");
      }, function(resp) {
