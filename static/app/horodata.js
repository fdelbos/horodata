...
$templateCache.put("horodata/menu/toolbar.html","<div><md-button ng-if=MainTitle().canEdit class=\"md-fab md-fab-bottom-right\" aria-label=Add ng-click=showAdd($event)><md-tooltip md-direction=left>Ajouter un tâche</md-tooltip><md-icon class=md-48 style=\"margin: -2px 0px 0px -1px\">access_time</md-icon></md-button><md-toolbar><div class=md-toolbar-tools><md-button hide-gt-sm class=md-icon-button aria-label=Settings ng-click=toggleSidenav()><md-icon class=md-24>menu</md-icon></md-button><h2><span>{{ MainTitle().title }}</span></h2><span flex></span><md-button class=\"md-fab md-mini\" aria-label=Favorite><span class=\"fa fa-user\"></span></md-button></div></md-toolbar></div>");
...
$templateCache.put("horodata/widgets/new_group.html","<md-button class=\"md-fab md-primary md-mini\" ng-click=showNewGroupDialog() aria-label=\"Nouveau Groupe\"><md-icon class=md-24>add</md-icon><md-tooltip md-direction=top>Ajouter un groupe</md-tooltip></md-button>");
...
$templateCache.put("horodata/widgets/configuration/customers.html","<div layout-margin layout-padding><div layout=column flex-gt-md=60 flex-sm=70><div layout=row layout-align=\"space-between center\"><div class=md-display-1>Dossiers</div><md-button ng-click=customers.create() class=\"md-raised md-primary\" hide show-gt-sm><md-icon class=md-24>add</md-icon>Ajouter</md-button><md-button ng-click=customers.create() class=\"md-fab md-primary\" hide-gt-sm aria-label=\"Ajouter un dossier\"><md-icon class=md-36>add</md-icon></md-button></div><p class=md-body-1>Un dossier peut representer un client ou un projet. Chaque nouvelle tache est attachee a un dossier.</p><md-whiteframe ng-if=\"group.customers.length == 0\" class=\"md-whiteframe-5dp md-accent\" layout-padding layout=column layout-align=\"center center\"><div class=\"md-headline input-error\">Aucune dossier enregistre</div><div>Vous devez definir des dossiers pour la saisie.</div></md-whiteframe><div layout=row ng-if=\"group.customers.length > 0\"><md-input-container flex><label>Selectionnez un dossiers pour l\'editer</label><md-select ng-model=customers.selected><md-option ng-repeat=\"c in group.customers\" value=\"{{ c.id }}\">{{ c.name }}</md-option></md-select></md-input-container><md-button ng-if=customers.selected ng-click=customers.edit() class=\"md-fab md-accent\" hide-gt-sm aria-label=\"Editer un dossier\"><md-tooltip md-direction=top>Modifier le dossier</md-tooltip><md-icon class=md-36>edit</md-icon></md-button></div></div></div>");
...
angular.module("horodata").controller("Group", [
  "$http", "$routeParams", "$scope", "titleService", "userService", "apiService", function($http, $routeParams, $scope, titleService, userService, apiService) {
    ...
    return $scope.$on("group.reload", function(e) {
      e.stopPropagation();
      return getGroup();
    });
  }
...
angular.module("horodata").directive("appWidgetsConfigurationCustomers", [
  "$mdDialog", "$mdMedia", function($mdDialog, $mdMedia) {
    ...
    l = function(scope, elem, attr) {
      var openCreateForm, openEditForm;
      openCreateForm = function(ev) {
        var fullscreen;
        fullscreen = $mdMedia('xs') || $mdMedia('sm');
        return $mdDialog.show({
  ...
  "$scope", "$mdDialog", "$mdToast", "$http", "$location", "apiService", function($scope, $mdDialog, $mdToast, $http, $location, apiService) {
    ...
    $scope.create = function() {
      return $http.post((apiService.get()) + "/groups/" + $scope.group.url + "/tasks", $scope.tasks.current).then(function(resp) {
        ...
        return $scope.errors = resp.tasks.errors;
      });
    };
    $scope.edit = function() {
