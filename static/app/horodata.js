...
$templateCache.put("horodata/widgets/listing.html","<div><div ng-if=!listing.data() layout=column layout-align=\"center center\"><md-progress-circular md-mode=indeterminate md-diameter=150></md-progress-circular><p>Chargement, veuillez patienter.</p></div><div ng-if=listing.data()><md-list class=md-body-1><md-list-item class=md-body-2 hide show-gt-md><div layout=row layout-align=\"space-between center\" flex><div flex=15>Jour</div><div ng-if=isAdmin flex=25>Utilisateur</div><div flex=25>Dossier</div><div flex=25>Tâche</div><div flex=10>Durée</div></div><md-divider></md-divider></md-list-item><md-list-item ng-repeat=\"i in listing.data()\" ng-click=\"showDetail($event, i)\" hide show-gt-md><div layout=row layout-align=\"space-between center\" flex><div flex=15>{{ i.created | Day }}</div><div flex=25 ng-if=isAdmin>{{ guests[i.creator_id].full_name }}</div><div flex=25>{{ customers[i.customer_id].name }}</div><div flex=25>{{ tasks[i.task_id].name }}</div><div flex=10>{{ i.duration | Duration }}</div></div><md-divider></md-divider></md-list-item><md-list-item ng-repeat=\"i in listing.data()\" class=md-2-line ng-click=\"showDetail($event, i)\" hide-gt-md><div class=md-list-item-text layout=column><div layout=row layout-align=\"space-between center\"><h3>{{ customers[i.customer_id].name }}</h3><h4>{{ i.created | Day }} - <span class=text-primary>{{ i.duration | Duration }}</span></h4></div><h4>{{ tasks[i.task_id].name }}</h4><span class=text-accent>{{ guests[i.creator_id].full_name }}</span></div><md-divider></md-divider></md-list-item></md-list></div></div>");
...
$templateCache.put("horodata/widgets/stats/customer_time.html","<div layout=column><app-widgets-loading ng-if=stats.loading()></app-widgets-loading><div ng-if=\"!stats.loading() && data.length > 0\" flex><div id=chart></div><c3chart bindto-id=chart ng-if=data><chart-column ng-repeat=\"i in data\" column-id=\"{{ customers[i.customer_id].name }}\" column-values=\"{{ i.duration }}\" column-type=pie></chart-column></c3chart></div><app-widgets-stats-no-data ng-if=\"!stats.loading() && data.length == 0\"></app-widgets-stats-no-data></div>");
...
$templateCache.put("horodata/widgets/stats/task_time.html","<div layout=column><app-widgets-loading ng-if=stats.loading()></app-widgets-loading><div ng-if=\"!stats.loading() && data.length > 0\" flex><div id=chart></div><c3chart bindto-id=chart ng-if=data><chart-column ng-repeat=\"i in data\" column-id=\"{{ tasks[i.task_id].name }}\" column-values=\"{{ i.duration }}\" column-type=pie></chart-column></c3chart></div><app-widgets-stats-no-data ng-if=\"!stats.loading() && data.length == 0\"></app-widgets-stats-no-data></div>");}]);
...
angular.module("horodata").controller("Group", [
  "$http", "$routeParams", "$scope", "titleService", "userService", "apiService", "groupNewService", "popupService", "listingService", function($http, $routeParams, $scope, titleService, userService, apiService, groupNewService, popupService, listingService) {
    ...
    $scope.$watch("search", function(v) {
      ...
      return listingService.listing().fetch(1);
    }, true);
    $scope.isAdmin = false;
    getGroup = function() {
...
angular.module('horodata').factory("listingService", [
  "$http", "apiService", function($http, apiService) {
    ...
    Listing = (function() {
      ...
      Listing.prototype.pages = function() {
        var res;
        if (this.total === -1) {
          return null;
        }
        res = {
          ...
          next: this.page * this.size > this.total ? null : this.page + 1
        };
        return res;
      };
...
angular.module("horodata").directive("appWidgetsStatsCustomerTime", [
  "statsService", function(statsService) {
    var l;
    l = function(scope) {
      scope.stats = statsService;
      return statsService.fetch(scope.group.url, "customer_time", scope.search, (function(_this) {
...
angular.module("horodata").directive("appWidgetsStatsGuestTime", [
  "statsService", function(statsService) {
    ...
    l = function(scope) {
      scope.stats = statsService;
      return statsService.fetch(scope.group.url, "guest_time", scope.search, (function(_this) {
        return function(data) {
          return scope.data = data;
        };
      })(this));
...
angular.module("horodata").directive("appWidgetsStatsTaskTime", [
  "statsService", function(statsService) {
    ...
    l = function(scope) {
      scope.stats = statsService;
      return statsService.fetch(scope.group.url, "task_time", scope.search, (function(_this) {
        return function(data) {
          return scope.data = data;
        };
      })(this));
