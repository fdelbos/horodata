...
angular.module("horodata").controller("BillingPlanChange", [
  "$scope", "$mdDialog", "$mdToast", "$http", "$location", "apiService", "groupService", function($scope, $mdDialog, $mdToast, $http, $location, apiService, groupService) {
    ...
    getCard();
    return w = $scope.$watchGroup(["addr.loaded", "card.loaded"], function(v) {
      console.log(v);
      if (!v[0] || !v[1]) {
        return;
      }
...
angular.module("horodata").controller("Billing", [
  "$scope", "titleService", function($scope, titleService) {
